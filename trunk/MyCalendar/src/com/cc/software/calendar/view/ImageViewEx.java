...
public class ImageViewEx extends ImageView {
    ...
    private GestureDetector mGestureDetector = new GestureDetector(new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onSingleTapConfirmed(MotionEvent e) {
            mSingleClickListener.onSinleClick();
            return isEmpty;
        };
    });
    ...
    private void applyInitMatrix(int imageWidth, int imageHeight) {
        ...
        if (imageWidth <= viewWidth && imageHeight <= viewHeight) {
            ...
            scale = Math.min((float) viewWidth / (float) imageWidth, (float) viewHeight / (float) imageHeight);
        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        case MotionEvent.ACTION_MOVE:
            if (mMode == DRAG) {
                ...
                mMatrix.postTranslate(event.getX() - mStartPt.x, event.getY() - mStartPt.y);
            } else if (mMode == ZOOM) {
    ...
    private void bounceBack() {
        ...
        mAnimation.prepareAnimation(startValues[Matrix.MTRANS_X], startValues[Matrix.MTRANS_Y], mInitTranslateX,
                        mInitTranslateY, startValues[Matrix.MSCALE_X], mInitScaleFactor, 0, 0);
    ...
    protected void onDoubleTaped(MotionEvent e) {
        ...
        if (values[Matrix.MSCALE_X] != mInitScaleFactor) {
            ...
            mAnimation.prepareAnimation(mInitScaleFactor, Math.max(mInitScaleFactor * 3, 1.0f), (int) e.getX(),
                            (int) e.getY());
            startAnimation(mAnimation);
            requestDisallowInterceptTouchEvent(true);
        }
    ...
    private void alignBorders(float[] values) {
        ...
        if (p.x != values[Matrix.MTRANS_X] || p.y != values[Matrix.MTRANS_Y]) {
            ...
            mAnimation.prepareAnimation(values[Matrix.MTRANS_X], values[Matrix.MTRANS_Y], p.x, p.y, scaleFactor,
                            scaleFactor, 0, 0);
    ...
    private class ImageTranlationAnimation extends Animation {
        ...
        public void prepareAnimation(float startX, float startY, float targetX, float targetY, float startScaleFactor,
                        float targetScaleFactor, int pivotX, int pivotY) {
        ...
        public void prepareAnimation(float startScaleFactor, float targetScaleFactor, int pivotX, int pivotY) {
            mMode = MODE_SCALE;
        ...
        protected void applyTransformation(float interpolatedTime, Transformation transformation) {
            ...
            if (mMode == MODE_BOUNCE_BACK) {
                ...
                float scaleFactor = 1 + (mTargetScaleFactor / mStartScaleFactor - 1) * interpolatedTime;
