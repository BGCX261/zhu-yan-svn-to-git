...
public class SetAlarm extends PreferenceActivity implements Alarms.AlarmSettings, TimePickerDialog.OnTimeSetListener {
    ...
    /**
     * Set an alarm. Requires an Alarms.ID to be passed in as an extra
     */
    @Override
    protected void onCreate(Bundle icicle) {
        ...
        /*
         * This should never happen, but does occasionally with the monkey. I
         * believe it's a race condition where a deleted alarm is opened before
         * the alarm list is refreshed.
         */
        ...
        getContentResolver().registerContentObserver(Alarms.AlarmColumns.CONTENT_URI, true, mAlarmsChangeObserver);
    ...
    protected Dialog onCreateDialog(int id) {
        ...
        case DIALOG_TIMEPICKER:
            d = new TimePickerDialog(SetAlarm.this, this, 0, 0, DateFormat.is24HourFormat(SetAlarm.this));
    ...
    /**
     * Alarms.AlarmSettings implementation. Database feeds current settings in
     * through this call
     */
    public void reportAlarm(int idx, boolean enabled, int hour, int minutes, Alarms.DaysOfWeek daysOfWeek,
                    boolean vibrate, String label, String alert) {
        ...
        if (Log.LOGV)
            Log.v("****** reportAlarm uri " + alert + " alert " + mAlarmPref.mAlert);
        updateTime();
        updateRepeat();
    ...
    private void saveAlarm(boolean popToast, String label) {
        if (mReportAlarmCalled && mAlarmPref.mAlert != null) {
            ...
            saveAlarm(this, mId, mAlarmOnPref.isChecked(), mHour, mMinutes, mDaysOfWeek, mVibratePref.isChecked(),
                            label, alertString, popToast);
        }
    }
    ...
    private static void saveAlarm(Context context, int id, boolean enabled, int hour, int minute,
                    Alarms.DaysOfWeek daysOfWeek, boolean vibrate, String label, String alert, boolean popToast) {
        if (Log.LOGV)
     ...
     */
    static void popAlarmSetToast(Context context, int hour, int minute, Alarms.DaysOfWeek daysOfWeek) {
     ...
     */
    static String formatToast(Context context, int hour, int minute, Alarms.DaysOfWeek daysOfWeek) {
        ...
        String daySeq = (days == 0) ? "" : (days == 1) ? context.getString(R.string.one_day) : context.getString(
        ...
        if (!(dispDays || dispHour || dispMinute)) {
            ...
            parts[0] = daySeq;
            parts[1] = !dispDays ? "" : dispHour && dispMinute ? context.getString(R.string.space) : !dispHour
                            && !dispMinute ? "" : context.getString(R.string.and);
        ...
        /*
         * if (Log.LOGV) Log.v("** TOAST daySeq " + daySeq + " hourSeq " +
         * hourSeq + " minSeq " + minSeq + " ret " + ret);
         */
        return ret;
    }
    ...
    void setTestAlarm() {
        ...
        saveAlarm(this, mId, true, hour, minutes, mDaysOfWeek, true, mLabel.getText(), mAlarmPref.mAlert.toString(),
                        true);
    }
