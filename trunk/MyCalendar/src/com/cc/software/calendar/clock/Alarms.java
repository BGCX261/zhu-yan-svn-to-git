...
public class Alarms {
    ...
    public final static String ALARM_ALERT_ACTION = "com.cc.software.calendar.clock.ALARM_ALERT";
    ...
    public static class AlarmColumns implements BaseColumns {
        ...
        public static final Uri CONTENT_URI = Uri
                .parse("content://com.cc.software.calendar.clock/alarm");
    ...
    static interface AlarmSettings {
        void reportAlarm(int idx, boolean enabled, int hour, int minutes,
     ...
     */
    public synchronized static void deleteAlarm(Context context, int alarmId) {
    ...
    public synchronized static Cursor getAlarmsCursor(
        ...
        return contentResolver.query(AlarmColumns.CONTENT_URI,
     ...
     */
    public synchronized static void getAlarms(ContentResolver contentResolver,
    ...
    private synchronized static void getAlarms(AlarmSettings alarmSettings,
            Cursor cur) {
        if (cur.moveToFirst()) {
            do {
                ...
                int daysOfWeek = cur
                        .getInt(AlarmColumns.ALARM_DAYS_OF_WEEK_INDEX);
                boolean enabled = cur.getInt(AlarmColumns.ALARM_ENABLED_INDEX) == 1 ? true
                        : false;
                boolean vibrate = cur.getInt(AlarmColumns.ALARM_VIBRATE_INDEX) == 1 ? true
                        : false;
                String message = cur
                        .getString(AlarmColumns.ALARM_MESSAGE_INDEX);
                String alert = cur.getString(AlarmColumns.ALARM_ALERT_INDEX);
                alarmSettings.reportAlarm(id, enabled, hour, minutes,
                        new DaysOfWeek(daysOfWeek), vibrate, message, alert);
     ...
     * alarmId
     */
    public synchronized static void getAlarm(ContentResolver contentResolver,
            AlarmSettings alarmSetting, int alarmId) {
     ...
     */
    public synchronized static void setAlarm(Context context, int id,
            boolean enabled, int hour, int minutes, DaysOfWeek daysOfWeek,
            boolean vibrate, String message, String alert) {
        ...
        if (Log.LOGV)
            Log.v("**  setAlarm * idx " + id + " hour " + hour + " minutes "
        ...
        resolver.update(
                ContentUris.withAppendedId(AlarmColumns.CONTENT_URI, id),
                values, null, null);

        int aid = disableSnoozeAlert(context);
        if (aid != -1 && aid != id)
            enableAlarmInternal(context, aid, false);
        setNextAlert(context);
    }
    ...
    public synchronized static void enableAlarm(final Context context,
    ...
    private synchronized static void enableAlarmInternal(final Context context,
            final int id, boolean enabled) {
        ContentResolver resolver = context.getContentResolver();
        ...
        class EnableAlarm implements AlarmSettings {
            ...
            public void reportAlarm(int idx, boolean enabled, int hour,
                    int minutes, DaysOfWeek daysOfWeek, boolean vibrate,
    ...
    static class AlarmCalculator implements AlarmSettings {
        ...
        public void reportAlarm(int idx, boolean enabled, int hour,
                ...
                String message, String alert) {
            if (enabled) {
                long atTime = calculateAlarm(hour, minutes, daysOfWeek)
                        .getTimeInMillis();
    ...
    static void enableAlert(Context context, int id, String label,
        ...
        if (Log.LOGV)
            Log.v("** setAlert id " + id + " atTime " + atTimeInMillis);
